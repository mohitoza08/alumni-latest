...
async function main() {
  ...
  const tables = [
    ...
    `CREATE TABLE IF NOT EXISTS colleges (
      ...
      name VARCHAR(255) NOT NULL, code VARCHAR(50) NOT NULL UNIQUE,
      address TEXT, city VARCHAR(100), state VARCHAR(100),
      country VARCHAR(100) DEFAULT 'India', website VARCHAR(255),
      logo_url VARCHAR(500), description TEXT, established_year INTEGER,
    ...
    `CREATE TABLE IF NOT EXISTS users (
      ...
      phone VARCHAR(20), profile_picture VARCHAR(500), bio TEXT,
      graduation_year INTEGER, current_year_level VARCHAR(10),
      degree VARCHAR(100), major VARCHAR(100),
      current_company VARCHAR(255), current_position VARCHAR(255),
      linkedin_url VARCHAR(255),
      status VARCHAR(20) DEFAULT 'active' CHECK (status IN ('active','inactive','pending','suspended','rejected')),
      email_verified BOOLEAN DEFAULT FALSE, last_login TIMESTAMP,
      created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP, updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
      ban_reason TEXT, suspended_until TIMESTAMP, onboarding_completed BOOLEAN DEFAULT FALSE,
    ...
    `CREATE TABLE IF NOT EXISTS user_sessions (
      id BIGINT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
      user_id BIGINT NOT NULL, token VARCHAR(255) NOT NULL UNIQUE,
    ...
    `CREATE TABLE IF NOT EXISTS email_otps (
      id BIGINT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
      email VARCHAR(255) NOT NULL, otp VARCHAR(6) NOT NULL,
    ...
    `CREATE TABLE IF NOT EXISTS applications (
      id BIGINT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
      college_id BIGINT NOT NULL, student_id BIGINT NOT NULL,
      full_name VARCHAR(255) NOT NULL, email VARCHAR(255) NOT NULL,
      graduation_year INTEGER NOT NULL, degree VARCHAR(100) NOT NULL, major VARCHAR(100),
      student_id_number VARCHAR(50), transcript_url VARCHAR(500),
      additional_documents TEXT,
      status VARCHAR(20) DEFAULT 'pending' CHECK (status IN ('pending','approved','rejected','under_review')),
      admin_notes TEXT, reviewed_by BIGINT, reviewed_at TIMESTAMP,
      created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP, updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
      FOREIGN KEY (college_id) REFERENCES colleges(id) ON DELETE CASCADE,
    ...
    `CREATE TABLE IF NOT EXISTS onboarding_requests (
      id BIGINT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
      user_id BIGINT NOT NULL, college_id BIGINT NOT NULL,
      type VARCHAR(20) NOT NULL CHECK (type IN ('student','alumni')),
      graduation_year INTEGER, current_year VARCHAR(20), semester VARCHAR(20),
      degree VARCHAR(100), major VARCHAR(100),
      current_company VARCHAR(255), current_position VARCHAR(255),
      linkedin_url VARCHAR(255), bio TEXT, phone VARCHAR(20),
      certificate_url VARCHAR(500),
      status VARCHAR(20) DEFAULT 'pending' CHECK (status IN ('pending','approved','rejected')),
      admin_notes TEXT, reviewed_by BIGINT, reviewed_at TIMESTAMP,
      professional_type VARCHAR(20) CHECK (professional_type IN ('corporate','business','freelancer','other')),
      business_name VARCHAR(255), business_type VARCHAR(100), freelancer_skills TEXT,
      created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP, updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
      FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE,
      FOREIGN KEY (college_id) REFERENCES colleges(id) ON DELETE CASCADE,
      FOREIGN KEY (reviewed_by) REFERENCES users(id) ON DELETE SET NULL
    ...
    `CREATE TABLE IF NOT EXISTS community_posts (
      id BIGINT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
      college_id BIGINT NOT NULL, author_id BIGINT NOT NULL,
      ...
      tags TEXT, image_url VARCHAR(500),
    ...
    `CREATE TABLE IF NOT EXISTS post_comments (
      id BIGINT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
      post_id BIGINT NOT NULL, author_id BIGINT NOT NULL,
      content TEXT NOT NULL, parent_comment_id BIGINT,
    ...
    `CREATE TABLE IF NOT EXISTS mentorships (
      id BIGINT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
      college_id BIGINT NOT NULL, mentor_id BIGINT NOT NULL, mentee_id BIGINT NOT NULL,
      status VARCHAR(20) DEFAULT 'active' CHECK (status IN ('pending','active','completed','cancelled')),
      request_message TEXT, start_date DATE, end_date DATE,
    ...
    `CREATE TABLE IF NOT EXISTS mentorship_requests (
      id BIGINT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
      college_id BIGINT NOT NULL, mentee_id BIGINT NOT NULL, mentor_id BIGINT NOT NULL,
      topic VARCHAR(255) NOT NULL, message TEXT NOT NULL,
    ...
    // 12. mentorship_sessions
    `CREATE TABLE IF NOT EXISTS mentorship_sessions (
      id BIGINT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
      mentorship_id BIGINT NOT NULL, session_date TIMESTAMP NOT NULL,
      duration_minutes INTEGER,
      meeting_type VARCHAR(20) CHECK (meeting_type IN ('in-person','video','phone','chat')),
      topics_discussed TEXT, action_items TEXT, mentor_notes TEXT, mentee_notes TEXT,
      created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    ...
    `CREATE TABLE IF NOT EXISTS events (
      id BIGINT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
      college_id BIGINT NOT NULL, organizer_id BIGINT NOT NULL,
      ...
      location VARCHAR(255), venue_address TEXT,
      is_virtual BOOLEAN DEFAULT FALSE, virtual_link VARCHAR(500),
      max_attendees INTEGER, registration_deadline TIMESTAMP,
    ...
    `CREATE TABLE IF NOT EXISTS event_registrations (
      ...
      registration_notes TEXT, attended_at TIMESTAMP,
      created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    ...
    // 15. fundraising_campaigns
    `CREATE TABLE IF NOT EXISTS fundraising_campaigns (
      id BIGINT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
      college_id BIGINT NOT NULL, created_by BIGINT NOT NULL,
      title VARCHAR(255) NOT NULL, description TEXT,
      goal_amount DECIMAL(15,2) NOT NULL, current_amount DECIMAL(15,2) DEFAULT 0.00,
      ...
      donor_count INTEGER DEFAULT 0, is_featured BOOLEAN DEFAULT FALSE,
      created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP, updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
      FOREIGN KEY (college_id) REFERENCES colleges(id) ON DELETE CASCADE,
    ...
    `CREATE TABLE IF NOT EXISTS donations (
      id BIGINT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
      campaign_id BIGINT NOT NULL, donor_id BIGINT,
      donor_name VARCHAR(255), donor_email VARCHAR(255),
      amount DECIMAL(15,2) NOT NULL, currency VARCHAR(3) DEFAULT 'USD',
      is_anonymous BOOLEAN DEFAULT FALSE, payment_method VARCHAR(50),
      transaction_id VARCHAR(255), message TEXT,
    ...
    `CREATE TABLE IF NOT EXISTS donation_requests (
      id BIGINT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
      campaign_id BIGINT NOT NULL, donor_id BIGINT,
      donor_name VARCHAR(255) NOT NULL, donor_email VARCHAR(255) NOT NULL,
      amount DECIMAL(15,2) NOT NULL, currency VARCHAR(3) DEFAULT 'USD',
      is_anonymous BOOLEAN DEFAULT FALSE, payment_method VARCHAR(50),
      transaction_reference VARCHAR(255) NOT NULL, receipt_url VARCHAR(500),
      message TEXT,
      status VARCHAR(20) DEFAULT 'pending' CHECK (status IN ('pending','verified','rejected')),
      admin_notes TEXT, verified_by BIGINT, verified_at TIMESTAMP,
      created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP, updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
      FOREIGN KEY (campaign_id) REFERENCES fundraising_campaigns(id) ON DELETE CASCADE,
      FOREIGN KEY (donor_id) REFERENCES users(id) ON DELETE SET NULL,
    ...
    `CREATE TABLE IF NOT EXISTS workshops (
      ...
      college_id BIGINT NOT NULL, organizer_id BIGINT NOT NULL,
      title VARCHAR(255) NOT NULL, description TEXT,
      instructor_name VARCHAR(255), instructor_bio TEXT,
      workshop_type VARCHAR(50) CHECK (workshop_type IN ('technical','soft_skills','career','entrepreneurship','leadership','other')),
      start_date TIMESTAMP NOT NULL, end_date TIMESTAMP, duration_hours INTEGER,
      location VARCHAR(255), is_virtual BOOLEAN DEFAULT FALSE, virtual_link VARCHAR(500),
      price DECIMAL(10,2) NOT NULL DEFAULT 0.00, currency VARCHAR(3) DEFAULT 'USD',
      max_attendees INTEGER, registration_deadline TIMESTAMP,
      banner_image VARCHAR(500), syllabus_url VARCHAR(500),
      status VARCHAR(20) DEFAULT 'upcoming' CHECK (status IN ('draft','upcoming','ongoing','completed','cancelled')),
      registration_count INTEGER DEFAULT 0, is_premium BOOLEAN DEFAULT TRUE,
      created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP, updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
      FOREIGN KEY (college_id) REFERENCES colleges(id) ON DELETE CASCADE,
      FOREIGN KEY (organizer_id) REFERENCES users(id) ON DELETE CASCADE
    )`,

    // 19. workshop_registrations
    `CREATE TABLE IF NOT EXISTS workshop_registrations (
      id BIGINT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
      workshop_id BIGINT NOT NULL, user_id BIGINT NOT NULL,
      status VARCHAR(20) DEFAULT 'pending' CHECK (status IN ('pending','verified','attended','cancelled','rejected')),
      payment_method VARCHAR(50), transaction_reference VARCHAR(255),
      payment_proof_url VARCHAR(500), amount_paid DECIMAL(10,2),
      registration_notes TEXT, verified_by BIGINT, verified_at TIMESTAMP,
    ...
    // 20. conversations (BEFORE messages because messages references it)
    `CREATE TABLE IF NOT EXISTS conversations (
      id BIGINT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
      title VARCHAR(255), created_by BIGINT NOT NULL, college_id BIGINT NOT NULL,
    ...
    `CREATE TABLE IF NOT EXISTS messages (
      ...
      mentorship_id BIGINT, conversation_id BIGINT,
      subject VARCHAR(255), content TEXT NOT NULL,
      is_read BOOLEAN DEFAULT FALSE, parent_message_id BIGINT,
      created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    ...
    `CREATE TABLE IF NOT EXISTS achievements (
      ...
      achievement_date DATE, organization VARCHAR(255),
      verification_url VARCHAR(500), is_verified BOOLEAN DEFAULT FALSE,
    ...
    `CREATE TABLE IF NOT EXISTS skills (
      id BIGINT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
      user_id BIGINT NOT NULL, college_id BIGINT NOT NULL,
      skill_name VARCHAR(255) NOT NULL, organization VARCHAR(255),
      date_obtained DATE, expiry_date DATE,
      created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
      FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE,
      FOREIGN KEY (college_id) REFERENCES colleges(id) ON DELETE CASCADE
    )`,
    ...
    `CREATE TABLE IF NOT EXISTS projects (
      ...
      technologies TEXT, project_link VARCHAR(500), date_completed DATE,
      created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
      FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE,
      FOREIGN KEY (college_id) REFERENCES colleges(id) ON DELETE CASCADE
    )`,

    // 25. resume_enhancements
    `CREATE TABLE IF NOT EXISTS resume_enhancements (
      id BIGINT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
      user_id BIGINT NOT NULL,
      original_resume TEXT, enhanced_resume TEXT,
      skills_suggested TEXT, improvements_made TEXT, ai_feedback TEXT,
      created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
      FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE
    )`,
    ...
    `CREATE TABLE IF NOT EXISTS subscriptions (
      ...
      payment_method VARCHAR(50), transaction_reference VARCHAR(255),
      amount_paid DECIMAL(10,2), auto_renew BOOLEAN DEFAULT FALSE,
      created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP, updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
      FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE
    )`,
    ...
    `CREATE TABLE IF NOT EXISTS notifications (
      ...
      title VARCHAR(255) NOT NULL, message TEXT, link_url VARCHAR(500),
      is_read BOOLEAN DEFAULT FALSE, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
      FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE
    )`,

    // 28. analytics_metrics
    `CREATE TABLE IF NOT EXISTS analytics_metrics (
      id BIGINT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
      college_id BIGINT NOT NULL, metric_date DATE NOT NULL,
      total_users INTEGER DEFAULT 0, total_alumni INTEGER DEFAULT 0,
      total_students INTEGER DEFAULT 0, new_registrations INTEGER DEFAULT 0,
      active_users INTEGER DEFAULT 0, total_posts INTEGER DEFAULT 0,
      total_events INTEGER DEFAULT 0, total_donations DECIMAL(15,2) DEFAULT 0.00,
    ...
    `CREATE TABLE IF NOT EXISTS user_badges (
      id BIGINT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
      user_id BIGINT NOT NULL, badge_id VARCHAR(50) NOT NULL,
      unlocked_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
      progress INTEGER DEFAULT 0, max_progress INTEGER,
      FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE,
    ...
    `CREATE TABLE IF NOT EXISTS user_streaks (
      ...
      last_checkin DATE, total_points INTEGER DEFAULT 0,
      posts_created INTEGER DEFAULT 0, comments_posted INTEGER DEFAULT 0,
      likes_received INTEGER DEFAULT 0, events_attended INTEGER DEFAULT 0,
  ...
  const indexes = [
    ...
    `CREATE INDEX IF NOT EXISTS idx_posts_college ON community_posts(college_id)`,
    ...
    `CREATE INDEX IF NOT EXISTS idx_mentorships_college ON mentorships(college_id)`,
    ...
    `CREATE INDEX IF NOT EXISTS idx_sessions_date ON mentorship_sessions(session_date)`,
    ...
    `CREATE INDEX IF NOT EXISTS idx_campaigns_college ON fundraising_campaigns(college_id)`,
    ...
    `CREATE INDEX IF NOT EXISTS idx_donation_requests_campaign ON donation_requests(campaign_id)`,
    ...
    `CREATE INDEX IF NOT EXISTS idx_donation_requests_date ON donation_requests(created_at DESC)`,
    `CREATE INDEX IF NOT EXISTS idx_workshops_college ON workshops(college_id)`,
    ...
    `CREATE INDEX IF NOT EXISTS idx_workshops_premium ON workshops(is_premium)`,
    ...
    `CREATE INDEX IF NOT EXISTS idx_achievements_college ON achievements(college_id)`,
    `CREATE INDEX IF NOT EXISTS idx_achievements_type ON achievements(achievement_type)`,
    `CREATE INDEX IF NOT EXISTS idx_skills_user ON skills(user_id)`,
    `CREATE INDEX IF NOT EXISTS idx_skills_college ON skills(college_id)`,
    `CREATE INDEX IF NOT EXISTS idx_projects_user ON projects(user_id)`,
    `CREATE INDEX IF NOT EXISTS idx_projects_college ON projects(college_id)`,
    `CREATE INDEX IF NOT EXISTS idx_resume_enhancements_user ON resume_enhancements(user_id)`,
    `CREATE INDEX IF NOT EXISTS idx_resume_enhancements_date ON resume_enhancements(created_at DESC)`,
    ...
    `CREATE INDEX IF NOT EXISTS idx_analytics_college ON analytics_metrics(college_id)`,
    ...
    `CREATE INDEX IF NOT EXISTS idx_user_streaks_points ON user_streaks(total_points DESC)`,
  ...
  const views = [
    ...
    `CREATE OR REPLACE VIEW v_recent_activity AS
     SELECT c.id AS college_id, c.name AS college_name,
       ...
       'post' AS activity_type, cp.title AS activity_title, cp.created_at
     FROM colleges c
     JOIN community_posts cp ON cp.college_id = c.id
     JOIN users u ON cp.author_id = u.id
     UNION ALL
     SELECT c.id, c.name, u.id, CONCAT(u.first_name, ' ', u.last_name),
       'event', e.title, e.created_at
     FROM colleges c JOIN events e ON e.college_id = c.id
     JOIN users u ON e.organizer_id = u.id
     ORDER BY created_at DESC`,

    `CREATE OR REPLACE VIEW v_leaderboard AS
     SELECT u.id AS user_id, CONCAT(u.first_name, ' ', u.last_name) AS user_name,
       u.role, u.profile_picture, u.college_id,
    ...
    `CREATE OR REPLACE VIEW v_pending_verifications AS
     SELECT 'donation' AS verification_type, dr.id, dr.campaign_id AS related_id,
       fc.title AS related_title, CONCAT(u.first_name, ' ', u.last_name) AS requester_name,
       dr.amount, dr.transaction_reference, dr.created_at, dr.status
     FROM donation_requests dr
     JOIN fundraising_campaigns fc ON dr.campaign_id = fc.id
     LEFT JOIN users u ON dr.donor_id = u.id
     WHERE dr.status = 'pending'
     UNION ALL
     SELECT 'workshop' AS verification_type, wr.id, wr.workshop_id AS related_id,
       w.title AS related_title, CONCAT(u.first_name, ' ', u.last_name) AS requester_name,
       wr.amount_paid AS amount, wr.transaction_reference, wr.created_at, wr.status
  ...
  for (const sql of tables) {
    try {
      await query(sql);
      totalOk++;
      console.log("  OK: " + sql.substring(sql.indexOf("TABLE ") + 6, sql.indexOf("(")).trim());
    } catch (e: any) {
      const msg = e.message || "";
      if (msg.includes("already exists") || e.code === "42P07") {
        totalOk++;
      } else {
