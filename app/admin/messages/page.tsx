...
import { Send, MessageSquare, Plus, UserPlus, ChevronDown, ChevronRight, CheckCheck, Check } from "lucide-react"
    ...
    try {
      const response = await fetch("/api/messages", {
        ...
        body: JSON.stringify({
          ...
          title: `Chat with ${selectedUser.first_name}`,
              ...
              <Tabs defaultValue="all">
                <TabsList className="w-full grid grid-cols-2">
                  ...
                  <TabsTrigger value="unread" className="text-xs">
                    Unread
                    {totalUnread > 0 && (
                      <span className="ml-1 text-green-500">{totalUnread}</span>
                    )}
                  </TabsTrigger>
                </TabsList>
                ...
                <TabsContent value="unread" className="mt-2">
                  <ScrollArea className="h-[calc(100vh-200px)]">
                    {groupedPeople.filter((g: any) => g.person.unread_count > 0).length === 0 ? (
                      <div className="p-8 text-center text-muted-foreground">
                      ...
                      <div className="divide-y divide-border">
                        {groupedPeople.filter((g: any) => g.person.unread_count > 0).map(({ person }) => (
                          <button
                            key={`unread-${person.other_user_id}`}
                            onClick={() => setSelectedConversation(person.id)}
                            className="w-full p-3 flex items-start gap-3 hover:bg-muted/50 transition-colors text-left font-medium"
                          >
                            <Avatar className="h-10 w-10">
                              <AvatarImage src={person.other_user_picture} />
                              <AvatarFallback>{getInitials(person.other_user_name || "U")}</AvatarFallback>
                            ...
                            <div className="flex-1 min-w-0">
                              <div className="flex items-center justify-between">
                                <span className="truncate">{person.other_user_name}</span>
                                <span className="text-xs text-muted-foreground ml-2 shrink-0">
                                  {person.last_message_at ? formatTime(person.last_message_at) : ""}
                                </span>
                              </div>
            ...
            {selectedConversation && currentConversation ? (
              ...
              <div className="flex-1 flex items-center justify-center text-muted-foreground">
                <div className="text-center">
                  ...
                  <p className="text-sm mt-1">Start chatting with students and alumni</p>
                </div>
              </div>
            )}
          </div>
